...
class Movie {
  ...
  validateCountries(countries = undefined) {
    if (countries === undefined) return console.warn("You must provide a list of countries.");
  ...
  validateGenres(genres = undefined) {
    if (genres === undefined) return console.warn("You must provide a list of genres.");
    ...
    for (let genre of genres){
      if (Movie.acceptedGenres.includes(genre)){
        ...
        console.warn(`${genre} is not an accepted genre, here's the list: ${Movie.acceptedGenres}`)
      } 
    }
  }
...
let testObject = {
  ...
  genres: ["Action", "NotAValidGenre", "Thriller"],
  score: 9.8
}
